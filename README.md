...
is the ANSI blue slot. In this theme that slot is too bright to carry the
row's text. The template gives the bar the theme's
lifted background instead. lazygit reads it as a second config file:

```bash
mkdir -p ~/.config/omarchy/themed
cp ~/.config/omarchy/themes/c64/themed/lazygit.yml.tpl \
  ~/.config/omarchy/themed/
omarchy theme set c64
```
